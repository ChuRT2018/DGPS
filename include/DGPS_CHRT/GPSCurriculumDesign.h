...
typedef struct DGPSHead {
	char messageType; //6
	short baseStation; //10
	double zCounter; //13
	char numOfSeq; //3 
	char lengthOfMessage; //5
	char healthState;//3
}DGPSHead;
...
typedef struct DGPSMessage {
	...
	double pseCorrect; //16
	double rangeRateCorrect;//8
	char ageOfData;//8
}DGPSMessage;


bool Findhead(unsigned char* pdata, int dataLength, int& headPosition);
bool DecodeHead(unsigned char* pdata, head& header);
...
bool RollByte(unsigned char input, int s, unsigned char& output);
bool StitchingWord(std::string input, std::string& output);
bool CheckParity(unsigned char* input);
bool DecodeDGPS(unsigned char* pdata, int length,DGPSHead& dgpsHead, std::unordered_map<char, DGPSMessage>& dgpsmessages);
